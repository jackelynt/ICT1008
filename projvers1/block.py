...
with open("sg_zipcode_mapper.csv","w",newline='') as file:
  writer = csv.writer(file)
  writer.writerow(["blk_no", "street", "road_name", "address", "latiude", "longtitude", "postal"])
...
punggolStreetList = [line.rstrip('\n') for line in open('PuggolStreetName.txt')]
...
while (number_of_returned_data >= 100):
    ...
    for data in jsonObj["result"]["records"]:
        with open("sg_zipcode_mapper.csv","a",newline='') as file:
            writer = csv.writer(file)
            if data["street"] in punggolStreetList:
                ...
                if onemapjson["found"] != 0:
                    datalist = [data["blk_no"], data["street"], onemapjson["results"][0]["ROAD_NAME"], onemapjson["results"][0]["ADDRESS"], onemapjson["results"][0]["LATITUDE"], onemapjson["results"][0]["LONGTITUDE"], onemapjson["results"][0]["POSTAL"]]
                    print(datalist)
